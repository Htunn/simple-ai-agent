...
class MCPManager:
    ...
    async def _create_stdio_transport(
        ...
        client = MCPClient(server_command=[command] + args, env=env)
    ...
    async def call_tool(
        ...
        if not transport:
            ...
            return {
                "content": [
                    {"type": "text", "text": f"Error: Server '{server_name}' not available"}
                ],
                "isError": True,
            }
    ...
    def get_server_info(self) -> dict[str, Any]:
        ...
        for server_name, transport in self.servers.items():
            info["servers"][server_name] = {
                ...
                "tools": [
                    name for name, server in self.tool_registry.items() if server == server_name
                ],
            }
